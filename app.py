...
for index, row in df.iterrows():
    try:
        ...
        expense1 = float(row['Spending Variance - Month 1'])
        expense2 = float(row['Spending Variance - Month 2'])
        expense3 = float(row['Spending Variance - Month 3'])
...
filtered_vendors = [v for v in score_df["Vendor"] if search_query in v.lower()] if search_query else score_df["Vendor"].tolist()
...
# --- Sidebar: CSV for Selected Vendor ---
